# 4Roads Backend

Backend API для интернет-магазина 4roads.su

## 🚀 Быстрый старт

### Установка зависимостей
```bash
pip install -r requirements.txt
```

### Запуск приложения
```bash
uvicorn app.main:app --reload
```

## 📦 Импорт данных

> 💡 **Подробная инструкция**: См. [QUICK_START.md](QUICK_START.md)

### Простой способ (shell-скрипт)
```bash
# Обычный импорт
./import_products.sh

# С обновлением изображений
./import_products.sh --refresh

# Тестовый запуск
./import_products.sh --dry-run --max-pages 1

# Очистка дублей
./import_products.sh --cleanup

# Справка
./import_products.sh --help
```

### Прямой запуск Python
```bash
# Импорт всех продуктов
python3 -m app.utils.import_4roads_full --collection-url https://4roads.su/collection/vse-kollektsii

# С опциями
python3 -m app.utils.import_4roads_full \
  --collection-url https://4roads.su/collection/vse-kollektsii \
  --max-pages 5 \
  --refresh-images
```

### Очистка неиспользуемых изображений
```bash
# Показать дубли
python3 -m app.utils.cleanup_orphaned_images

# Удалить дубли
python3 -m app.utils.cleanup_orphaned_images --execute
```

## 📚 Документация

- 🚀 [QUICK_START.md](QUICK_START.md) - **Быстрый старт: импорт продуктов**
- 🛠️ [IMPORT_IMAGES_FIX.md](IMPORT_IMAGES_FIX.md) - Исправление багов с дублированием изображений
- 👨‍💼 [ADMIN_PANEL.md](ADMIN_PANEL.md) - Документация по админ-панели
- 📝 [CONTINUITY.md](CONTINUITY.md) - История разработки

## 🏗️ Структура проекта

```
app/
├── api/v1/             # API endpoints
├── core/               # Бизнес-логика
│   ├── dto/           # Data Transfer Objects
│   ├── repositories/  # Репозитории для работы с БД
│   └── services/      # Сервисы бизнес-логики
├── infrastructure/     # Инфраструктурный слой
│   ├── config/        # Конфигурация
│   ├── database/      # Модели БД и подключения
│   ├── errors/        # Кастомные ошибки
│   └── middleware/    # Middleware
└── utils/             # Утилиты
```

## 🛠️ Утилиты

### import_4roads_full.py
Полноценный импорт продуктов с сайта 4roads.su:
- Парсинг HTML страниц
- Извлечение данных о продуктах
- Скачивание и оптимизация изображений
- Создание/обновление записей в БД

### cleanup_orphaned_images.py
Очистка неиспользуемых файлов изображений:
- Поиск файлов, отсутствующих в БД
- Подсчет занимаемого места
- Безопасное удаление с dry-run режимом

## 🗄️ База данных

### Миграции
```bash
# Создать новую миграцию
alembic revision --autogenerate -m "описание изменений"

# Применить миграции
alembic upgrade head

# Откатить последнюю миграцию
alembic downgrade -1
```

## 🎯 Принципы разработки

- **SOLID** - соблюдение принципов объектно-ориентированного программирования
- **KISS** - Keep It Simple, Stupid
- **Clean Code** - чистый, читаемый и поддерживаемый код
- **Design Patterns** - использование проверенных паттернов проектирования

## 🐛 Известные проблемы и исправления

### ✅ Исправлено: Дублирование изображений при импорте
**Проблема**: При повторном импорте с флагом `--refresh-images` создавались новые файлы, а старые оставались на диске.

**Решение**: Добавлена явная очистка файлов перед обновлением изображений. Подробности в [IMPORT_IMAGES_FIX.md](IMPORT_IMAGES_FIX.md)

## 📝 Лицензия

Проприетарное ПО. Все права защищены.

